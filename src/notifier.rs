...
pub struct EventNotifier(mio::Waker);
...
impl EventAwaiter {
    pub fn new() -> io::Result<(Self, EventNotifier)> {
        let poll = mio::Poll::new()?;
        let waker = mio::Waker::new(poll.registry(), mio::Token(0))?;
        Ok((Self { poll }, EventNotifier(waker)))
    }
...
mod tests {
    ...
    #[test]
    fn test_waker() {
        let (mut awaiter, notifier) =
            EventAwaiter::new().expect("could not initialize event awaiter");
